...
export function parseSellerPage(html, sellerId) {
  ...
  if (notFoundPatterns.some((p) => businessName.includes(p) || html.includes(`<h1>${p}</h1>`) || html.includes(`<h2>${p}</h2>`))) {
    return { sellerId, businessName: '', rating: null, ratingOutOf: null, reviewCount: null, email: '', phone: '', companyName: '', address: '', zipCode: '', city: '', kvkNumber: '', vatNumber: '', sellerDataSection: {} };
  }
